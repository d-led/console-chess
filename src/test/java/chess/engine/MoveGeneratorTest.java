...
class MoveGeneratorTest {
  ...
  void pawnSinglePush() {
    List<Move> moves = generator.generateLegalMoves(board, Color.OUTLINE);
    assertThat(moves).extracting(Move::toString).contains("e2e3", "d2d3");
  }

  @Test
  void pawnDoublePushFromStart() {
    List<Move> moves = generator.generateLegalMoves(board, Color.OUTLINE);
    assertThat(moves).extracting(Move::toString).contains("e2e4", "d2d4");
  }
  ...
  void knightHasCorrectMovesFromStart() {
    ...
    assertThat(moves).extracting(Move::toString).contains("b1a3", "b1c3");
    // g1 knight can go to f3 and h3
    assertThat(moves).extracting(Move::toString).contains("g1f3", "g1h3");
  }
  ...
  void noMovesThroughOwnPieces() {
    ...
    List<Move> moves = generator.generateLegalMoves(board, Color.OUTLINE);
    assertThat(moves).extracting(Move::toString).doesNotContain("a1a3", "a1a4");
  }

  @Test
  void kingHasNoMovesInitially() {
    List<Move> moves = generator.generateLegalMoves(board, Color.OUTLINE);
    assertThat(moves).extracting(Move::toString).doesNotContain("e1e2", "e1d1", "e1f1");
  }
  ...
  void stalemateIsDetected() {
    ...
    // Clear the board
    for (int r = 0; r < 8; r++) for (int f = 0; f < 8; f++) stalemate.removePiece(new Square(f, r));
