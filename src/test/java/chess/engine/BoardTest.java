...
class BoardTest {
  ...
  void initialBoardHasCorrectSetup() {
    ...
    assertThat(board.pieceAt(Square.from("b1")))
        .contains(new Piece(Color.OUTLINE, PieceType.KNIGHT));
    assertThat(board.pieceAt(Square.from("c1")))
        .contains(new Piece(Color.OUTLINE, PieceType.BISHOP));
    assertThat(board.pieceAt(Square.from("d1")))
        .contains(new Piece(Color.OUTLINE, PieceType.QUEEN));
    assertThat(board.pieceAt(Square.from("e1"))).contains(new Piece(Color.OUTLINE, PieceType.KING));
    assertThat(board.pieceAt(Square.from("f1")))
        .contains(new Piece(Color.OUTLINE, PieceType.BISHOP));
    assertThat(board.pieceAt(Square.from("g1")))
        .contains(new Piece(Color.OUTLINE, PieceType.KNIGHT));
    assertThat(board.pieceAt(Square.from("h1"))).contains(new Piece(Color.OUTLINE, PieceType.ROOK));

    // White pawns
    for (char f = 'a'; f <= 'h'; f++) {
      assertThat(board.pieceAt(Square.from("" + f + "2")))
          .contains(new Piece(Color.OUTLINE, PieceType.PAWN));
    }
    ...
    // Black pawns
    for (char f = 'a'; f <= 'h'; f++) {
      assertThat(board.pieceAt(Square.from("" + f + "7")))
