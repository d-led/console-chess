...
/**
 * Greedy tactical engine: always captures the highest-value piece,
 * prefers checks, otherwise picks a random move. ELO ~500 — fun to beat.
 */
public class GreedyEngine implements ChessEngine {

    private final Random random;

    public GreedyEngine() { this(new Random()); }
    public GreedyEngine(Random random) { this.random = random; }

    @Override public String name() { return "Greedy (~ELO 500)"; }
    ...
    public Optional<Move> selectMove(GameState game) {
        ...
        if (!captures.isEmpty()) {
            ...
            captures.sort((a, b) -> Integer.compare(capturedValue(game.board(), b), capturedValue(game.board(), a)));
