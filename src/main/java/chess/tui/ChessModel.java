...
public class ChessModel implements Model {
    ...
    private static final Style LIGHT_SQ = Style.newStyle()
        .background(Color.color("235")).foreground(Color.color(FG));
    private static final Style DARK_SQ = Style.newStyle()
        .background(Color.color("240")).foreground(Color.color(FG));
    private static final Style SEL = Style.newStyle()
        .background(Color.color("100")).foreground(Color.color(FG));
    ...
    private static final Style CURSOR = Style.newStyle()
        .background(Color.color("33")).foreground(Color.color(FG));
    ...
    private static final Map<String, String> CURSOR_KEYS = Map.ofEntries(
        Map.entry("up",    "0,1"),  Map.entry("k", "0,1"),   Map.entry("K", "0,1"),
        Map.entry("down",  "0,-1"), Map.entry("j", "0,-1"),  Map.entry("J", "0,-1"),
        Map.entry("left",  "-1,0"), Map.entry("h", "-1,0"),  Map.entry("H", "-1,0"),
        Map.entry("right", "1,0"),  Map.entry("l", "1,0"),   Map.entry("L", "1,0")
    );
    ...
    private CellStyle cellAppearance(int file, int rank, boolean hasSelection,
            boolean isCursor, boolean isLegalDest, boolean isSelected) {
        boolean isLight = (file + rank) % 2 == 0;
        Style base = isLight ? LIGHT_SQ : DARK_SQ;

        if (isSelected)                      return new CellStyle(SEL, "[", "]", null);
        if (isCursor && hasSelection && !isLegalDest)
            return new CellStyle(base, " ", " ", "⛝");
        if (isCursor && hasSelection && isLegalDest)
            return new CellStyle(LEGAL, "▸", "◂", null);
    ...
    private void renderStatus(StringBuilder sb) {
        String line = switch (game.status()) {
