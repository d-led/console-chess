...
public class MoveGenerator {
    ...
    return switch (piece.type()) {
      ...
      case BISHOP ->
          generateSlidingMoves(
              board, from, piece.color(), new int[][] {{1, 1}, {1, -1}, {-1, 1}, {-1, -1}});
      case ROOK ->
          generateSlidingMoves(
              board, from, piece.color(), new int[][] {{1, 0}, {-1, 0}, {0, 1}, {0, -1}});
      case QUEEN ->
          generateSlidingMoves(
              board,
              from,
  ...
  private void addPawnCaptures(
      Board board, Square from, Color color, PawnParams p, List<Move> moves) {
  ...
  private List<Move> generateSlidingMoves(
      Board board, Square from, Color color, int[][] directions) {
    List<Move> moves = new ArrayList<>();
